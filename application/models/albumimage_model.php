...
//        $query=$this->db->get("anima_albumimage")->result();
        $query=$this->db->query("SELECT * FROM `anima_albumimage` WHERE `photographeralbum`='$id' ORDER BY `order`")->result();
        return $query;
    }
	...
	{
		$query=$this->db->query("SELECT `image` FROM `anima_albumimage` WHERE `id`='$id'")->row();
		return $query;
	}
}
?>
